...
Процедура ВыполнитьАвтозаполнение() Экспорт
    ...
    НоменклатураАбонентки = Константы.ВКМ_НоменклатураАбоненстаяПлата.Получить();
    НоменклатураРабот = Константы.ВКМ_НоменклатураРаботыСпециалиста.Получить();
    
    Если Не ЗначениеЗаполнено(НоменклатураАбонентки) Или Не ЗначениеЗаполнено(НоменклатураРабот) Тогда
        ВызватьИсключение "Не заполнены необходимые константы номенклатуры!";
    КонецЕсли;
    
    Услуги.Очистить();
    
    АбоненсткаяПлатаИзДоговора = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Договор, "ВКМ_СуммаАбонентскойПлаты");
    Если Договор.ВКМ_СуммаАбонентскойПлаты > 0 Тогда
        НоваяСтрока = Услуги.Добавить();
        НоваяСтрока.Номенклатура = НоменклатураАбонентки;
        НоваяСтрока.Количество = 1;
        НоваяСтрока.Цена = АбоненсткаяПлатаИзДоговора;
        НоваяСтрока.Сумма = АбоненсткаяПлатаИзДоговора;
    КонецЕсли;
